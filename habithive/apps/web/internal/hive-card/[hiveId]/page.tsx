//apps/web/internal/hive-card/[hiveId]/page.tsx
...
async function getHiveCardData(hiveId: string) {
  ...
  const daysSinceStart = Math.max(
    1,
    Math.floor((Date.now() - hive.startedAt.getTime()) / (1000 * 60 * 60 * 24))
  );
...
function ProgressBar({ value }: { value: number }) {
  ...
  return (
    <div style={{ fontFamily: "monospace", fontSize: 28, letterSpacing: 2 }}>
...
export default async function HiveCardPage({
  ...
  // Secret token check — only Puppeteer passes this
  const expectedToken = process.env.INTERNAL_SCREENSHOT_TOKEN ?? "habithive-internal";
  if (searchParams.token !== expectedToken) {
    return notFound();
  }
        ...
        >
          ...
          >
            ...
            <div style={{ textAlign: "right" }}>
              ...
              <p style={{ color: "#64748b", fontSize: 12 }}>habithive.app</p>
            </div>
          </div>
        </div>
      </body>
